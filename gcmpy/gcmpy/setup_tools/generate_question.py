...
class Question:
    ...
    def should_ask(self, question_dict):
        if self.follows_up:
            for prev_question, accepted_answers in self.follows_up:
                if question_dict[prev_question].answer in accepted_answers or ('any' in accepted_answers and question_dict[prev_question].answer != None) or ('was_not_asked' in accepted_answers):
                    return True
            return False
        return True
    ...
    def load_question(self, question_dict):

        # check if the yaml entry should be skipped
        if not self.should_ask(question_dict):
            return 

        # if should_ask() returns true, call correct questionary API
        elif self.q_type == 'text':
            answer = questionary.text(self.prompt, default=self.default, validate=QuestionValidator.val_text).ask()
