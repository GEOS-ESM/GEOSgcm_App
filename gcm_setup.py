...
pathdict['QCONFIG']     = os.path.join(pathdict['ETCDIR'], 'gcm_setup_config.yaml')
[log(key + ": " + pathdict[key])for key in pathdict]

# Environment variables to be exported at script end
envdict = {}
...
class exceptions:
    ...
    def printusage():
            print(pathdict['SCRIPTNAME']+ ', a setup script for the GEOS GCM' + textdecor.NEWLINE + textdecor.NEWLINE + \
                    textdecor.INDENT + 'Usage: '+ pathdict['SCRIPTNAME'] + ' [optional flag]' \
                    + textdecor.NEWLINE + textdecor.NEWLINE + \
                    textdecor.INDENT + '   --link         Link GEOSgcm.x into experiment directory' + textdecor.NEWLINE + \
                    textdecor.INDENT + '   --singularity  Set up Singularity experiment' + textdecor.NEWLINE + \
                    textdecor.INDENT + '-h --help         Show usage' + 
                    textdecor.NEWLINE + textdecor.NEWLINE + \
                    textdecor.INDENT + 'If invoked alone, the script runs as normal.' + textdecor.NEWLINE + textdecor.NEWLINE + \
                    textdecor.INDENT + 'For more information, please contact Matt Thompson or Scott Rabenhorst.' + textdecor.NEWLINE)
            exit(1)
            # log(printusage())
...
def main():
    #######################################################################
    #        Install Directory Validation and Command Line Arguments
    #######################################################################
    ...
    if not os.path.exists(os.path.join(pathdict['BINDIR'],'GEOSgcm.x')):
        exceptions.raisefatalexception('You are trying to run ' + \
                                textdecor.decorateexe(pathdict['SCRIPTNAME']) + \
                                ' in the ' + textdecor.decoratepath(pathdict['OWD']) + \
    ...
    GEOSTAG = open(os.path.join(pathdict['ETCDIR'],'.AGCM_VERSION'),'r').read()
    log(GEOSTAG)

    # Loop through arguments and match to each case
    ...
    for arg in enumerate(sys.argv[1:]):
        ...
        if arg[-1] == '--link':
            ...
            exceptions.raiseuserexception('Command line argument \"' + arg[-1] + '\" not recognized.' + textdecor.NEWLINE + \
                                'See usage:' + textdecor.NEWLINE)
            exceptions.printusage()
    ...
    envdict['SITE'] = open(os.path.join(pathdict['ETCDIR'],'SITE.rc'),'r').read().split()[-1]
    # log('Node: ' + NODE)
    # log('Architecture: ' + ARCH)
    # log('Site: ' + envdict['SITE'])

    #######################################################################
    #                 Test for Compiler and MPI Setup
    #######################################################################
    # Extract BASEDIR tail
    envdict['BASEDIR'] = pathdict['BASEDIR'] = open(os.path.join(pathdict['ETCDIR'],'BASEDIR.rc'),'r').read().split()[-1]
    # log(pathdict['BASEDIR'])

    # Map MPI dirname to correct MPI implementation
    MPI = os.path.split(pathdict['BASEDIR'])[-1]
    # log(MPI)
    # MPI = '3208238dskfjld' # DEBUG
    if any(mpi in MPI for mpi in ['openmpi','hpcx']): MPI = 'openmpi'
    elif any(mpi in MPI for mpi in ['intelmpi','impi']): MPI = 'intelmpi'
    elif 'mvapich2' in MPI: MPI = 'mvapich2'
    elif 'mpich' in MPI: MPI = 'mpich'
    elif 'mpt' in MPI: MPI = 'mpt'
    else: MPI='intelmpi' # Assume default is Intel MPI in case of older baselibs
    log(textdecor.NEWLINE + 'MPI implementation is: ' + MPI + textdecor.NEWLINE)

    #######################################################################
    #                     Read gcm_setup_config.yaml
    #######################################################################
    qconfig = yaml.safe_load(open(pathdict['QCONFIG'],'r'))
    [log(q) for q in qconfig['mainquestions'][-1]]

    #[print(str(q) + ' | ' + str(p)) for q,p in qconfig['mainquestions'][-1]['options'].items()]
    log(type(qconfig['mainquestions'][-1]['default']))


    # #######################################################################
    #               Enter Experiment Specific Run Parameters
    #######################################################################
    #   TODO        YAML        Implemented    
    # ---------------------------------------------------------------------
                    # echo
...
# Need to handle keyboard interrupts to override default questionary handling 
# which just skips the question.
if __name__ == '__main__':
    try:
        main()
    except KeyboardInterrupt as e:
        exceptions.raiseuserexception('Interrupted by user! Please try again.')
        exceptions.cleanup()
        exit(130)