...
set ocmos = `ls -1 $GEOSS2S/runx/${YRI}/${MD}/ens${ENS_MEMBER}/MOM_Output`
mkdir -p $ARCHIVE/GEOS_S2S/seasonal/Y$YRI/${MD}/ens${ENS_MEMBER}/MOM_Output
@ noc1 = 0
@ noc2 = 0
foreach ocmo ( $ocmos )
    echo "COPY $ocmo to archive..."
    scp -p $GEOSS2S/runx/${YRI}/${MD}/ens${ENS_MEMBER}/MOM_Output/$ocmo $ARCHIVE/GEOS_S2S/seasonal/Y$YRI/${MD}/ens${ENS_MEMBER}/MOM_Output
    set ocstatus = $status
    @ noc1 = $noc1 + $ocstatus
    @ noc2 = $noc2 + 1
end
...
if ($nhld1 == $nhld2) then
        if ( $nhld1 != 25 ) exit
        if ( $nhld1 == 25 ) echo "HOLDING OK, PROCEED WITH CLEANING"
        /bin/rm -f $GEOSS2S/runx/${YRI}/${MD}/ens${ENS_MEMBER}/holding/geosgcm_*/${capend6}/${MD}.geosgcm_*.${capend6}01_??00z.nc4
