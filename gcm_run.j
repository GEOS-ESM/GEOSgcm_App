...
{{ MOM6 }}cp -f  $HOMDIR/MOM_input .
{{ MOM6 }}cp -f  $HOMDIR/MOM_override .
{{ CICE6 }}cp -f  $HOMDIR/ice_in .
...
{{ COUPLED }}set HEARTBEAT_DT = `grep '^\s*HEARTBEAT_DT:' CAP.rc | cut -d: -f2 | awk '{print $1}'`
